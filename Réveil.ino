...
void loop() {
  ...
  ec3 = digitalRead(colone3);
    ...
    while (!digitalRead(colone1)){    //----------111111111111111111111111111111111111111111111111111111111
         if(appui>=200){
          digitalWrite(r,vr);
          digitalWrite(g,vg);
          digitalWrite(b,vb);
...
/*if(eminut>=30 && reveil==0 && repet ==1 && now.hour()==eheure && now.minute()<eminut){    //--------a verifier eminut >30 veut dire que que l'heure voulu est 1h de plus et minute entre 0 et 30 ex 07h00 -30 = 6h30
  ...
  if(reveil==0 && repet==1 && now.hour()==heure && now.minute()>25){    //marche pas si reveil = 15h00 et time = 14h59 pas meme heur donc pas rouge
    vr=255;
    vg=0;
    vb=0;
  }
